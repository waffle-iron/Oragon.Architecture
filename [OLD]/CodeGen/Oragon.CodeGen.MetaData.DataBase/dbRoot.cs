...
// #define PLUGINS_FROM_SUBDIRS  // if defined Plugins can also live in subdirectories of the Oragon.CodeGen.MetaData.DataBase-bin-dir
/*
...
namespace Oragon.CodeGen.MetaData.DataBase
{
#if ENTERPRISE
	using System.Runtime.InteropServices;
	...
	/// <summary>
	/// Oragon.CodeGen.MetaData.DataBase is the root of the Oragon.CodeGen.MetaData.DataBase meta-data. Oragon.CodeGen.MetaData.DataBase is an intrinsic object available to your script and configured based on the settings
	/// you have entered in the Default Settings dialog. It is already connected before you script execution begins.
	/// </summary>
	/// <remarks>
	///	Oragon.CodeGen.MetaData.DataBase has 1 Collection:
	/// <list type="table">
	...
	///
	/// This can be very useful, you might need more meta data than Oragon.CodeGen.MetaData.DataBase supplies, in fact,
	/// Oragon.CodeGen.MetaData.DataBase will eventually offer extended meta data using this feature as well. The current plan
	/// is that any extended data added via MyGeneration will have a key that beings with "Oragon.CodeGen.MetaData.DataBase.Something"
	...
	///	<code>
	///	Oragon.CodeGen.MetaData.DataBase.Connect "SQL", "Provider=SQLOLEDB.1;Persist Security Info=True;User ID=sa;Data Source=localhost"
	///	
	///	Oragon.CodeGen.MetaData.DataBase.DbTarget	= "SqlClient"
	///	Oragon.CodeGen.MetaData.DataBase.DbTargetMappingFileName = "C:\Program Files\MyGeneration\Settings\DbTargets.xml"
	///	
	/// Oragon.CodeGen.MetaData.DataBase.Language = "VB.NET"
	/// Oragon.CodeGen.MetaData.DataBase.LanguageMappingFileName = "C:\Program Files\MyGeneration\Settings\Languages.xml"
	/// 
	/// Oragon.CodeGen.MetaData.DataBase.UserMetaDataFileName = "C:\Program Files\MyGeneration\Settings\UserMetaData.xml"
	/// </code>
	...
	/// only go ahead and set the Oragon.CodeGen.MetaData.DataBase.DbTarget in your script thus overriding the Default Settings.
	/// </example>
...
#endif 
    {
		...
		private void Reset()
		{
			...
			UserData.AppendChild(UserData.CreateNode(XmlNodeType.Element, "Oragon.CodeGen.MetaData.DataBase", null));
		...
		public IDbConnection BuildConnection(string driver, string connectionString) 
		{
			...
			switch(driver.ToUpper())
			{
                ...
                case MyMetaDrivers.VistaDB:	
					try
					{
						Oragon.CodeGen.MetaData.DataBase.VistaDB.MetaHelper mh = new Oragon.CodeGen.MetaData.DataBase.VistaDB.MetaHelper();
		...
		/// <summary>
		/// This is how you connect to your DBMS system using Oragon.CodeGen.MetaData.DataBase. This is already called for you before your script beings execution.
		/// </summary>
		...
		///		<item><term>"DB2"</term><description>Provider=IBMDADB2.1;Password=sa;User ID=DB2Admin;Data Source=Oragon.CodeGen.MetaData.DataBase;Persist Security Info=True</description></item>	
            ...
            try
            {
                ...
                switch (_driver)
                {
                    case dbDriver.SQL:
                        ...
                        ClassFactory = new Oragon.CodeGen.MetaData.DataBase.Sql.ClassFactory();
                        break;
                    ...
                    case dbDriver.Oracle:
                        ...
                        this.StripTrailingNulls = false;
                        this.requiredDatabaseName = true;
                        ClassFactory = new Oragon.CodeGen.MetaData.DataBase.Oracle.ClassFactory();
                        break;
                    ...
                    case dbDriver.Access:
                        ...
                        ClassFactory = new Oragon.CodeGen.MetaData.DataBase.Access.ClassFactory();
                        break;
                    ...
                    case dbDriver.MySql:
                        ...
                        this.requiredDatabaseName = true;
                        ClassFactory = new Oragon.CodeGen.MetaData.DataBase.MySql.ClassFactory();
                        break;
                    ...
                    case dbDriver.MySql2:
                        ...
                        this.StripTrailingNulls = true;
                        this.requiredDatabaseName = true;
                        ClassFactory = new Oragon.CodeGen.MetaData.DataBase.MySql5.ClassFactory();
                        break;
                    ...
                    case dbDriver.DB2:
                        ...
                        this.StripTrailingNulls = false;
                        this.requiredDatabaseName = false;
                        ClassFactory = new Oragon.CodeGen.MetaData.DataBase.DB2.ClassFactory();
                        break;
                    ...
                    case dbDriver.ISeries:
                        ...
                        this.StripTrailingNulls = false;
                        this.requiredDatabaseName = false;
                        ClassFactory = new Oragon.CodeGen.MetaData.DataBase.ISeries.ClassFactory();
                        break;
                    ...
                    case dbDriver.Pervasive:
                        ...
                        this.StripTrailingNulls = false;
                        this.requiredDatabaseName = false;
                        ClassFactory = new Oragon.CodeGen.MetaData.DataBase.Pervasive.ClassFactory();
                        break;
                    ...
                    case dbDriver.PostgreSQL:
                        ...
                        this.StripTrailingNulls = false;
                        this.requiredDatabaseName = false;
                        ClassFactory = new Oragon.CodeGen.MetaData.DataBase.PostgreSQL.ClassFactory();
                        break;
                    ...
                    case dbDriver.PostgreSQL8:
                        ...
                        this.StripTrailingNulls = false;
                        this.requiredDatabaseName = false;
                        ClassFactory = new Oragon.CodeGen.MetaData.DataBase.PostgreSQL8.ClassFactory();
                        break;
                    ...
                    case dbDriver.Firebird:
                        ...
                        this.StripTrailingNulls = false;
                        this.requiredDatabaseName = false;
                        ClassFactory = new Oragon.CodeGen.MetaData.DataBase.Firebird.ClassFactory();
                        break;
                    ...
                    case dbDriver.SQLite:
                        ...
                        this.StripTrailingNulls = false;
                        this.requiredDatabaseName = false;
                        ClassFactory = new Oragon.CodeGen.MetaData.DataBase.SQLite.ClassFactory();
                        break;
#if !IGNORE_VISTA
                    case dbDriver.VistaDB:
                        ...
                        try
                        {
                            ...
                            this.StripTrailingNulls = false;
                            this.requiredDatabaseName = false;
                            ClassFactory = new Oragon.CodeGen.MetaData.DataBase.VistaDB.ClassFactory();
                        }
                        catch
                        {
                            throw new Exception("Invalid VistaDB connection or VistaDB not installed");
                    ...
                    case dbDriver.Advantage:
                        ...
                        this.StripTrailingNulls = false;
                        this.requiredDatabaseName = false;
                        ClassFactory = new Oragon.CodeGen.MetaData.DataBase.Advantage.ClassFactory();
                    ...
                    case dbDriver.Plugin:
                        ...
                        ClassFactory = new Oragon.CodeGen.MetaData.DataBase.Plugin.ClassFactory(plugin);
                        break;
		...
		/// <summary>
		/// True if Oragon.CodeGen.MetaData.DataBase has been successfully connected to your DBMS, False if not.
		/// </summary>
		...
		/// <summary>
		/// Returns Oragon.CodeGen.MetaData.DataBase's current dbDriver enumeration value as defined by its current connection.
		/// </summary>
		...
		/// <summary>
		/// Returns Oragon.CodeGen.MetaData.DataBase's current DriverString as defined by its current connection.
		/// </summary>
		/// <remarks>
        ...
        private IDbConnection GetConnectionFromPlugin(string providerName, string pluginConnectionString, out IMyMetaPlugin plugin)
        {
            ...
            if (!Plugins.ContainsKey(providerName))
            {
                throw new Exception("Oragon.CodeGen.MetaData.DataBase Plugin \"" + providerName + "\" not registered.");
            }
            else
            {
            ...
            get
            {
                if (plugins == null)
                {
                    ...
                    if (info.Exists)
                    {
                        ...
                        foreach (FileInfo dllFile in info.Directory.GetFiles("Oragon.CodeGen.MetaData.DataBase.Plugins.*.dll",SearchOption.AllDirectories))
#else                        
                        foreach (FileInfo dllFile in info.Directory.GetFiles("Oragon.CodeGen.MetaData.DataBase.Plugins.*.dll"))
#endif                        
                        {
                        	try
                        	{
...
//                        	throw new ApplicationException("Cannot load Plugin(s) " + fileNames.ToString(), err);
		...
		{ 
			get
			{
				return @"//Oragon.CodeGen.MetaData.DataBase";
			} 
		}
		...
		internal bool GetXmlNode(out XmlNode node, bool forceCreate)
		{
			...
			if(null == _xmlNode)
			{
				...
				{
					_xmlNode = UserData.CreateNode(XmlNodeType.Element, "Oragon.CodeGen.MetaData.DataBase", null);
					UserData.AppendChild(_xmlNode);
				}
				else
				{
					_xmlNode = UserData.SelectSingleNode("./Oragon.CodeGen.MetaData.DataBase");
				}
			}
        ...
        public static void MergeUserMetaDataFiles(string f1, string db1, string f2, string db2, string fm) 
        {
            ...
            XmlNode newDocRoot = newDoc.SelectSingleNode("//Oragon.CodeGen.MetaData.DataBase/Databases/Database[@p='" + db1 + "']");

            XmlDocument otherDoc = new XmlDocument();
            otherDoc.Load(f2Inf.FullName);
            XmlNode otherDocRoot = otherDoc.SelectSingleNode("//Oragon.CodeGen.MetaData.DataBase/Databases/Database[@p='" + db2 + "']");

            MergeXml(newDoc, newDocRoot, "//Oragon.CodeGen.MetaData.DataBase/Databases/Database[@p='" + db1 + "']", otherDoc, otherDocRoot, "//Oragon.CodeGen.MetaData.DataBase/Databases/Database[@p='" + db2 + "']");
		...
		/// <summary>
		/// Returns all of the languages for a given driver, regardless of Oragon.CodeGen.MetaData.DataBase's current connection
		/// </summary>
		/// <returns>An array with all of the possible languages.</returns>
		...
		/// <summary>
		/// Returns all of the dbTargets for a given driver, regardless of Oragon.CodeGen.MetaData.DataBase's current connection
		/// </summary>
		/// <returns>An array with all of the possible dbTargets.</returns>
	...
	/// <summary>
	/// The current list of support dbDrivers. Typically VBScript and JScript use the string version as defined by Oragon.CodeGen.MetaData.DataBase.DriverString.
	/// </summary>
#if ENTERPRISE
