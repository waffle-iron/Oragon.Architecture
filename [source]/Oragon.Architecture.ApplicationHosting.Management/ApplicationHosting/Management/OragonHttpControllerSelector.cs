...
namespace Oragon.Architecture.ApplicationHosting.Management
{
	...
	public static class Error
	{
		...
		internal static Exception AmbiguousController()
		{
			throw new NotImplementedException();
		}

		internal static Exception ControllerNameAmbiguous_WithRouteTemplate()
		{
			throw new NotImplementedException();
		}
	}
	public class CandidateAction
	{
		...
		{
			return string.Format(System.Globalization.CultureInfo.CurrentCulture, "{0}, Order={1}, Prec={2}", new object[]
			{
	...
	public class OragonHttpControllerSelector : IHttpControllerSelector
	{
		...
		/// <summary> Initializes a new instance of the <see cref="T:System.Web.Http.Dispatcher.OragonHttpControllerSelector" /> class.</summary>
		/// <param name="configuration">The configuration.</param>
		public OragonHttpControllerSelector()
		{
			HttpConfiguration configuration = ManagementHostStartup.WebApiHttpConfiguration;
		...
		public virtual HttpControllerDescriptor SelectController(HttpRequestMessage request)
		{
			...
			if (routeData != null)
			{
				directRouteController = OragonHttpControllerSelector.GetDirectRouteController(routeData);
			...
			{
				throw new HttpResponseException(request.CreateErrorResponse(HttpStatusCode.NotFound, new Exception()));
			}
			...
			throw OragonHttpControllerSelector.CreateAmbiguousControllerException(request.GetRouteData().Route, controllerName, controllerTypes);
		}
		...
		internal static CandidateAction[] GetDirectRouteCandidates(IHttpRouteData routeData)
		{
			...
			if (subRoutes == null)
			{
				return GetDirectRouteCandidates(routeData.Route);
			}
			List<CandidateAction> list = new List<CandidateAction>();
			foreach (IHttpRouteData current in subRoutes)
			{
				CandidateAction[] directRouteCandidates = GetDirectRouteCandidates(current.Route);
		...
		public static CandidateAction[] GetDirectRouteCandidates(IHttpRoute route)
		{
			...
			if (dataTokens.TryGetValue<HttpActionDescriptor[]>("actions", out array2) && array2 != null && array2.Length > 0)
			{
				array = array2;
			}
			if (array == null)
			...
			decimal num2;
			if (dataTokens.TryGetValue<decimal>("precedence", out num2))
			{
				precedence = num2;
			}
			HttpActionDescriptor[] array3 = array;
			for (int i = 0; i < array3.Length; i++)
			{
				ReflectedHttpActionDescriptor actionDescriptor = (ReflectedHttpActionDescriptor)array3[i];
